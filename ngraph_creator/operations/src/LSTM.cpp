...
std::shared_ptr<ngraph::Node> LSTM::createNode() {
    ...
    if (!isCIFGenabled && !isValidInputTensor(9)) {
        isCIFGenabled = true;
    ...
    if (isProjectionUsed)
        proj_clipping = sModelInfo->ParseOperationInput<float>(mNnapiOperationIndex, 21);
    ...
    if (isLayerNormUsed) {
        ...
        auto i_t_transposed = transpose(NC_CN, i_t);
        auto f_t_transposed = transpose(NC_CN, f_t);
        auto c_t_transposed = transpose(NC_CN, c_t);
        auto o_t_transposed = transpose(NC_CN, o_t);

        const auto& elementType = inputNode->get_element_type();
        auto reduceAxes = ngraph::op::Constant::create(ngraph::element::i32, ngraph::Shape{}, {1});
        ...
        if (!isCIFGenabled) {
            i_t_Mean =
                std::make_shared<ngraph::opset3::ReduceMean>(i_t_transposed, reduceAxes, false);
            i_t_Variance = calculateVariance(i_t_transposed, i_t_Mean, reduceAxes, elementType);
        }
        f_t_Mean = std::make_shared<ngraph::opset3::ReduceMean>(f_t_transposed, reduceAxes, false);
        f_t_Variance = calculateVariance(f_t_transposed, f_t_Mean, reduceAxes, elementType);

        c_t_Mean = std::make_shared<ngraph::opset3::ReduceMean>(c_t_transposed, reduceAxes, false);
        c_t_Variance = calculateVariance(c_t_transposed, c_t_Mean, reduceAxes, elementType);

        o_t_Mean = std::make_shared<ngraph::opset3::ReduceMean>(o_t_transposed, reduceAxes, false);
        o_t_Variance = calculateVariance(o_t_transposed, o_t_Mean, reduceAxes, elementType);
...
std::shared_ptr<ngraph::Node> LSTM::calculateVariance(
    ...
    ngraph::Output<ngraph::Node> diff = std::make_shared<ngraph::opset3::Subtract>(
        transpose(NC_CN, input), mean, ngraph::op::AutoBroadcastType::NUMPY);
    diff = std::make_shared<ngraph::opset3::ReduceSum>(
        std::make_shared<ngraph::opset3::Multiply>(transpose(NC_CN, diff), transpose(NC_CN, diff)),
        reduction_axes, false);
