...
Blob::Ptr NnapiModelInfo::GetInOutOperandAsBlob(RunTimeOperandInfo& op, const uint8_t* buf,
    ...
    } else if (op.type == OperandType::TENSOR_QUANT8_SYMM || op.type == OperandType::TENSOR_QUANT8_SYMM_PER_CHANNEL) {
        ALOGV("check if tensors of type TENSOR_QUANT8_SYMM/TENSOR_QUANT8_SYMM_PER_CHANNEL supported");
